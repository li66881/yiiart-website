...
export default function StorefrontControls() {
  ...
  return (
    <div ref={controlsRef} className="relative flex items-center border border-stone-300 bg-white text-xs text-stone-700 sm:text-sm">
      <button
        id="yiiart-language"
        type="button"
        ...
        className="flex h-8 items-center gap-1.5 border-r border-stone-200 px-2 outline-none transition-colors hover:bg-stone-100"
      >
        <FlagIcon code={selectedLanguage.flagCode} label={selectedLanguage.name} />
        <span>{selectedLanguage.label}</span>
        <span aria-hidden="true" className="text-stone-400">v</span>
      </button>
      ...
      <button
        ...
        className="flex h-8 min-w-0 items-center gap-1.5 px-2 outline-none transition-colors hover:bg-stone-100"
      >
