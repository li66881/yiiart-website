...
test("sorts approved product media by explicit order and storefront role", () => {
  ...
  assert.deepEqual(media.map((item) => item.id), ["front", "room", "video"])
  assert.equal(media[2].type, "video")
  assert.equal(media[2].posterUrl, "https://assets.yiiart.com/poster.webp")
...
test("uses room scene as primary when no white-bg front/original exists", () => {
  const media = getApprovedProductMedia({
    productMedia: [
      { _key: "detail", mediaType: "image", role: "detail", url: "https://assets.yiiart.com/detail.webp", approvedForStorefront: true },
      { _key: "room", mediaType: "image", role: "living_room", url: "https://assets.yiiart.com/room.webp", approvedForStorefront: true },
    ],
  })

  assert.equal(media[0].id, "room")
  assert.equal(media[1].id, "detail")
})
...
test("keeps legacy image galleries working when audited media is absent", () => {
  ...
  assert.equal(media[1].role, "detail")
